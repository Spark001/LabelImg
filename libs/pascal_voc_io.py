    ...
    def appendObjects(self, top):
        for each_object in self.boxlist:
            ...
            if each_object['type'] == 'Rect':
                ...
                xmin.text = str(each_object['xmin']-1)
                ymin = SubElement(bndbox, 'ymin')
                ymin.text = str(each_object['ymin']-1)
                xmax = SubElement(bndbox, 'xmax')
                xmax.text = str(each_object['xmax']-1)
                ymax = SubElement(bndbox, 'ymax')
                ymax.text = str(each_object['ymax']-1)
            elif each_object['type'] == 'Point':
                point = SubElement(object_item, 'point')
                xmin = SubElement(point, 'x')
                xmin.text = str(each_object['x']-1)
                ymin = SubElement(point, 'y')
                ymin.text = str(each_object['y']-1)
            elif each_object['type'] == 'Polygon':
                polygon = SubElement(object_item, 'polygon')
                x1 = SubElement(polygon, 'x1')
                x1.text = str(each_object['x1']-1)
                y1 = SubElement(polygon, 'y1')
                y1.text = str(each_object['y1']-1)
                x2 = SubElement(polygon, 'x2')
                x2.text = str(each_object['x2']-1)
                y2 = SubElement(polygon, 'y2')
                y2.text = str(each_object['y2']-1)
                x3 = SubElement(polygon, 'x3')
                x3.text = str(each_object['x3']-1)
                y3 = SubElement(polygon, 'y3')
                y3.text = str(each_object['y3']-1)
                x4 = SubElement(polygon, 'x4')
                x4.text = str(each_object['x4']-1)
                y4 = SubElement(polygon, 'y4')
                y4.text = str(each_object['y4']-1)
