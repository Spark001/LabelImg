...
class MainWindow(QMainWindow, Window):
	...
	def __init__(self):
		...
		self.labelsuffix = '.txt'
		self.choice = None
		self.verigyColor = QColor(192,253,123)
	...
	## Actions
		...
		openNextImgandNo = action('&No and Next', self.openNextImg('No'),
	...
	## Store actions for further handling.
		...
		self.actions.beginner = (
			...
			openNextImgandYes,openNextImgandNo, openPrevImg, save,
			None,
	...
	def openFile(self, _value=False):
		...
		filters = "Image & Label files (%s)" % \
				  ' '.join(formats + ['*%s' % self.labelsuffix])
	...
	def toggleDone(self, filePath):
		...
		savedFileName = os.path.splitext(imgFileName)[0] + self.labelsuffix
	...
	def loadLabels(self, labelPath):
		...
		assert len(labels) == 1
		#TODO change canvas color according to the label
		self.setClean()
