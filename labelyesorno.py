...
__appname__ = 'Choose the YES or NO !'
...
class MainWindow(QMainWindow, Window):
	...
	def __init__(self):
		...
		candidateNum = 2
		candidateText = ['open','closed']
		if os.path.exists('./predefined.txt'):
			with open('./predefined.txt', 'r') as f:
				lines = f.readlines()
				lines = [l.strip() for l in lines]
				assert len(lines) == 1
				candidateText = lines[0].split(',')
				candidateNum = len(candidateText)
	...
	## Layout
		...
		self.labelWidget = SingleChoice(candidateNum, candidateText)
	...
	## Actions
		...
		self.yessignal.connect(partial(self.labelWidget.setstate, 0))
		self.nosignal.connect(partial(self.labelWidget.setstate, 1))
		openNextImgandYes = action('&Yes and Next', partial(self.openNextImg, 'Yes'),
								   'd', 'next', u'Marked Yes and Open Next Image')

		openNextImgandNo = action('&No and Next', partial(self.openNextImg, 'No'),
								  'f', 'next', u'Marked No and Open Next Image')

		openPrevImg = action('&Prev Image', self.openPrevImg,
							 'e', 'prev', u'Open Prev')
		openNextImg = action('&Next Image', self.openNextImg,
								  'r', 'next', u'Marked No and Open Next Image')
	...
	## Store actions for further handling.
		...
		self.actions.beginner = (
			...
			openNextImgandYes,openNextImgandNo, openNextImg, openPrevImg, save,
			None,
